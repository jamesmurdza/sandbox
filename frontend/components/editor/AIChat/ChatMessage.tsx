...
export default function ChatMessage({
  ...
  const [applyProgress, setApplyProgress] = useState<{current: number, total: number, currentFile?: string}>({current: 0, total: 0})
  const [applyResults, setApplyResults] = useState<{success: string[], failed: string[]}>({success: [], failed: []})
  ...
  const waitForFileToLoad = (filePath: string, expectedContent: string): Promise<boolean> => {
    return new Promise((resolve) => {
      ...
      const checkFileLoaded = () => {
        ...
        const contentLoaded = expectedContent === '' ? true : (editorContent === expectedContent || editorContent.length > 0)
  ...
  const applyAllCodeBlocks = async (blocks: CodeBlockData[]) => {
    ...
    try {
      setApplyProgress({current: 0, total: uniqueFiles.length, currentFile: undefined})
      ...
      for (let i = 0; i < uniqueFiles.length; i++) {
        ...
        setApplyProgress({current: i + 1, total: uniqueFiles.length, currentFile: fileName})
        ...
        try {
          ...
          if (existingOpenTab && existingOpenTab.id === activeFileIdRef.current && editorRef?.current) {
          ...
          while (!fileActivated && activationAttempts < 20) {
            if (activeFileIdRef.current === cleanPath) {
              fileActivated = true
              console.log(`✅ File activated: ${fileName} (activeFileId: ${activeFileIdRef.current})`)
          ...
          if (!fileActivated) {
            console.error(`❌ Failed to activate file: ${fileName}, activeFileId: ${activeFileIdRef.current}, expected: ${cleanPath}`)
          ...
          while (!decorationsVisible && attempts < 10) {
            const hasDecorations = editorRef?.current?.getModel()?.getAllDecorations()?.some((d: any) => 
          ...
          if (!decorationsVisible) {
            console.warn(`⚠️ No diff decorations visible for ${fileName} after ${attempts} attempts`)
          }
          
          // CRITICAL: Save decoration state before moving to next file
          if (i < uniqueFiles.length - 1) { // Not the last file
             ...
             if (currentFileState && model && (model as any).granularDiffState) {
               console.log(`💾 Saving granular state for ${fileName} before switching`)
               ...
               if (granularState?.blocks) {
                 console.log(`✅ Saved ${granularState.blocks.length} diff blocks for ${fileName}`)
               }
             } else {
               console.warn(`⚠️ Could not save state for ${fileName}: fileState=${!!currentFileState}, model=${!!model}, granularState=${!!(model as any)?.granularDiffState}`)
             }
          ...
          const modifiedContent = editorRef?.current?.getValue() || latestBlock.code
        ...
        {message.role === "assistant" ? (
          ...
          <ReactMarkdown remarkPlugins={[remarkGfm]} components={components}>
            ...
            {(() => {
              ...
              // Calculate unique files from tracked code blocks
              const uniqueFilePaths = new Set(
                codeBlocksRef.current.map(b => b.intendedFile.replace(' (new file)', ''))
              )
              ...
              const showApplyAll = uniqueFileCount > 1 && modifiedFiles.length === 0
              ...
              return (
                <div className="mt-4 pt-4 border-t border-border/30">
                    ...
                    <>
                        ...
                        {applyingAll ? (
                          <>
                            ...
                            {applyProgress.total > 0 
                              ? `Applying ${applyProgress.current}/${applyProgress.total}${applyProgress.currentFile ? `: ${applyProgress.currentFile}` : ''}...`
                              : 'Applying Changes...'
                            }
                          </>
                        ) : (
                          <>
                            ...
                            Apply All ({uniqueFileCount} {uniqueFileCount === 1 ? 'file' : 'files'})
                          </>
                        )}
                      </Button>
                      ...
                      {!applyingAll && (
                        <p className="text-xs text-muted-foreground mt-2 text-center">
                          Will apply changes to: {[...uniqueFilePaths].map(f => f.split('/').pop()).join(', ')}
                        </p>
                      )}
                    ...
                    >
                      {savingAll ? (
                        ...
                        <>
                          ...
                          Save All ({modifiedFiles.length} {modifiedFiles.length === 1 ? 'file' : 'files'})
                        </>
                      )}
                    </Button>
                  )}
                  ...
                  {/* Results display */}
                  {(applyResults.success.length > 0 || applyResults.failed.length > 0) && !applyingAll && !showSaveAll && (
